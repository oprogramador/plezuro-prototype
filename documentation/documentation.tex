...
\center
\huge
\pl{Programowanie obiektowe i graficzne, Bazy danych}
\en{Objective and graphical programming, Database}
\\
...
\pl{Temat: Baza NO-SQL}
\en{Topic: NO-SQL database}
\\
...
\\
	\begi
		\item
			Piotr Sroczkowski\\
			\pl{Pomysł, język skryptowy, IDE, prawie wszystko}
			\en{Idea, scripting language, IDE, almost all}
		\item
			Daniel Mikulski\\
			\pl{Testowanie, całkowanie numeryczne}
			\en{Tests, numerical integration}
	\enit
...
\subsubsection{
	\pl{Kolejność operatorów (od tych wykonywanych na końcu)}
	\en{Operator precedence (from those executed at the end)}
}
...
\begin{enumerate}
	\item
		...
		\en{Install any distribution of GNU/Linux operating system (next instructions for Debian derivatives).
		You can use}
	...
	\item
		\pl{Zainstaluj git, chociaż powinien być wbudowany w twojej dystrybucji}
		\en{Install git, although it should be built-in in your distribution}
	...
	\item
		...
		: \textit{./make.sh}
	\item
		\pl{Przejdź folder wyżej}
		\en{Go one directory level up}
		: \textit{cd ..}
	\item
		\pl{Uruchom}
		\en{Run}
