...
namespace Mondo.Engine {
	class Tokenizer {
		...
		private TokenTypes detectType(int i) {
			if(General.Converges( StrB, SetSymbol, i )) return TokenTypes.DollarOpen;
			if(General.Converges( StrB, DicSymbol, i )) return TokenTypes.HashOpen;
		...
		private Token match(ref int i, TokenTypes t) {
			...
			switch(t) {
				...
				case TokenTypes.DollarOpen:	 	ob = SetSymbol; i+=SetSymbol.Length;	break;
				case TokenTypes.HashOpen:	 	ob = DicSymbol; i+=DicSymbol.Length;		break;
